...
public class Diialogos2Final : MonoBehaviour
{
    ...
    private void Update()
    {
        if (activeDialog && Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.Joystick1Button1) && activeDialog)
        {
            ...
            {
                EmpezarDialogo();
                Debug.Log("sssasasfsdf");
            }
            else if (viñeta.GetComponentInChildren<TextMeshPro>().text == lineasDialogo[LineIndex])
            {
                NextDialogLine();
    ...
    private void NextDialogLine()
    {
        ...
        else
        {
            ...
            Eventos.eve.PasarNivel.Invoke(7);
